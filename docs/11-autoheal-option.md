...
- 모듈 종료 상태: **Auto-heal 규칙 설정 완료·재활용 관찰됨** (모듈 09 수행자는 Easy Auth 비활성 상태)

---

## 0단계 — (선택) 변수 재설정

> ⏭️ **이전 모듈(08·09 또는 10)에서 이어서 같은 터미널로 진행 중이라면 이 단계는 건너뛰세요.**
...
모듈 09에서 Easy Auth를 활성화했다면 — 모듈 10(사이드카)을 건너뛴 경우에도 — 활성 상태일 수 있습니다. curl 검증을 위해 멱등으로 비활성화합니다.
...
3단계에서 기록한 `started_at` 값보다 나중 시각이면 워커 프로세스 재활용이 확인된 것입니다.

🖼️ **예상 화면** — Azure Portal → 해당 App Service → **"문제 진단 및 해결"** → **"Auto heal"** 검색 → Auto-heal 이벤트 타임라인에서 재활용 이벤트를 확인합니다.
