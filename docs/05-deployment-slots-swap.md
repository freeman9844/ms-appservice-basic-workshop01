...
| 증상 | 원인 | 해결 방법 |
|------|------|-----------|
| `sed` 실행 후에도 `grep '^VERSION' app.py`가 `VERSION = "v1"`을 표시함 | 줄 끝 문자(CRLF), UTF-8 BOM 또는 실제 코드 형식이 치환 패턴과 일치하지 않습니다. | `cat -A app.py \| grep VERSION`으로 숨김 문자를 확인하고 실제 줄 형식에 맞게 `sed` 패턴을 조정합니다. |
| 스테이징 슬롯의 `curl` 응답이 느리거나 502 오류가 발생함 | 슬롯을 처음 배포한 뒤 콜드스타트가 진행 중입니다. | 배포 완료 후 30–60초 기다리거나 `curl` 요청을 다시 실행합니다. |
| 슬롯 스왑 명령이 오랫동안 완료되지 않음 | App Service가 스왑 전에 새 슬롯의 각 인스턴스에 기본 `/` 요청을 보내 응답을 기다립니다. 앱 시작 시간이 길면 수 분이 걸릴 수 있습니다. | 200 응답을 강제하려면 3단계의 `WEBSITE_SWAP_WARMUP_PING_PATH`와 `WEBSITE_SWAP_WARMUP_PING_STATUSES`를 설정합니다.<br>Portal의 **배포 슬롯 → 슬롯 교환**에서 진행 상태를 확인합니다. |
