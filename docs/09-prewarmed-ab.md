...
이 선택 심화 모듈에서는 07의 Azure Monitor Autoscale을 **Automatic Scaling**으로 전환하고, `hey`와 두 Python observer를 사용하여 부하 시작 뒤 새 instance가 **언제 실제 응답에 처음 투입되는지**를 관찰합니다. 단일 실행의 승패를 가르기보다 `Prewarmed=0`과 Microsoft 권장 기본값인 `Prewarmed=1`에서 보이는 외부 증거를 기록하고 해석합니다.

- 07의 Autoscale 설정을 제거하고 Automatic Scaling(Maximum burst 5, Always ready 1, Prewarmed 1)을 활성화합니다.
- `STARTUP_DELAY_SECONDS=30`으로 새 프로세스의 시작 준비 시간을 눈에 보이게 만듭니다.
- `hey` 부하 시작 시각과 새 instance의 최초 관찰 시각으로 `load_to_first_response_seconds`를 계산합니다.
- `/api/info`의 `started_at`으로 계산한 `first_response_age`는 30초 시작 지연이 적용됐는지 확인하는 보조 지표로 유지합니다.
...
| Web App | Prewarmed | 다음 HTTP scale-out에 준비할 워밍 버퍼 수 | Trial A 0 / Trial B 1 |
...
뒤의 A/B 실험에서는 다른 Automatic Scaling 값은 유지한 채 `Prewarmed`만 0과 1로 변경하여, buffer가 없는 상태와 Microsoft가 대부분의 운영 시나리오에 권장하는 기본값에서 새 instance가 실제 응답에 투입되는 시점을 비교합니다.
...
이번 모듈의 주 관찰 포인트는 **부하를 시작한 뒤 새 instance 응답이 처음 보이기까지 얼마나 걸렸는가**입니다. `STARTUP_DELAY_SECONDS=30`으로 시작 준비 시간을 눈에 보이게 만들고, 동일한 burst 부하에서 `Prewarmed=0`과 `Prewarmed=1`의 관찰 결과를 같은 형식으로 기록합니다.

`load_to_first_response_seconds`는 `hey` 시작 직전 기록한 `load_started_at`부터 observer가 새 instance 응답을 처음 받은 `first_seen_at`까지의 시간입니다. 이것이 Prewarmed의 주 비교 지표입니다. `started_at`은 30초 시작 지연 전에 기록되므로 `first_response_age`는 약 30–40초의 준비 시간이 실제로 적용됐는지 보는 보조 지표입니다. `first_seen_at`은 플랫폼 내부의 정확한 activation 시각이 아니라 클라이언트가 처음 관찰한 외부 증거입니다.

🟢 **실행 — 시작 지연 설정과 결과 경로 준비**

> 👁️ 두 시험의 결과를 저장할 디렉터리와 JSON 파일 경로를 먼저 준비하고, 시작 준비 시간을 관찰할 수 있도록 `STARTUP_DELAY_SECONDS=30`을 앱 설정에 추가합니다.
...
# cold-start worker와 rolling warm buffer의 차이를 관찰하기 위해 앱 시작 지연을 30초로 설정합니다.
az webapp config appsettings set -g "$RG" -n "$APP" \
  --settings STARTUP_DELAY_SECONDS=30 --output none &&
echo "STARTUP_DELAY_SECONDS=30 설정 완료"
```
...
시험 B는 반드시 시험 A의 부하가 끝나고 새 기준 상태가 다시 확보된 뒤 시작합니다. 이 게이트는 scale-in 자체를 관찰하려는 것이 아니라, 시험 A로 늘어난 인스턴스가 남아 있으면 시험 B에서 scale-out이 일어나지 않아 비교가 무효가 되므로 **두 시험이 같은 단일 인스턴스 기준선에서 시작하도록 보장하는 통제 장치**입니다. 기준선을 확보한 뒤 Prewarmed를 권장 기본값 1로 설정하고, 같은 burst에서 부하 시작 기준 최초 응답 지연을 다시 관찰합니다.
...
> 👁️ 시험 B는 Microsoft가 대부분의 운영 시나리오에 권장하는 기본값 `Prewarmed=1`을 사용합니다. 이 설정은 유휴 상태에서 1개를 항상 실행하는 값이 아니라 HTTP 부하가 시작된 뒤 유지할 rolling buffer 크기입니다. 할당된 Prewarmed instance는 초 단위 과금 대상입니다.

```bash
# 시험 B를 위해 Prewarmed rolling buffer를 권장 기본값 1로 설정합니다.
az rest --method patch \
  --uri "${APP_ID}/config/web?api-version=2024-11-01" \
  --body '{"properties":{"minimumElasticInstanceCount":1,"preWarmedInstanceCount":1}}' \
  --output none &&
...
> 4. `hey -z 180s -c 100 -q 10`으로 시험 A와 동일한 180초 부하를 백그라운드 실행합니다. 출력 파일은 `$AB_DIR/hey-burst-1.out`을 사용합니다.
...
if BASELINE_INSTANCE=$(curl -fsS --max-time 10 "$APP_URL/api/info" |
  ...
  echo "Prewarmed=1 load_started_at: $PREWARM_LOAD_STARTED_AT"
...
metric_timestamp	observed_at	instance_count
instance	load_started_at	first_seen_at	load_to_first_response_seconds	started_at	first_response_age
2026-07-25T09:13:00Z	2026-07-25T09:13:35Z	1
d09f4aa4	2026-07-25T09:13:03Z	2026-07-25T09:13:50Z	47	2026-07-25T09:13:15Z	35
c0b2201f	2026-07-25T09:13:03Z	2026-07-25T09:13:52Z	49	2026-07-25T09:13:17Z	35
3122a953	2026-07-25T09:13:03Z	2026-07-25T09:13:57Z	54	2026-07-25T09:13:23Z	34
5bef3ff3	2026-07-25T09:13:03Z	2026-07-25T09:14:05Z	62	2026-07-25T09:13:32Z	33
2026-07-25T09:14:00Z	2026-07-25T09:14:37Z	5
2026-07-25T09:16:00Z	2026-07-25T09:16:40Z	5
observer exit=0, hey exit=0, metric exit=0
```

> 👁️ 기준 instance를 제외한 새 instance 4개가 기록됐고 `InstanceCount`도 최종 5까지 증가했으며 세 exit code가 모두 0이므로 유효한 시험 B 결과입니다. 새 instance는 부하 시작 47·49·54·62초 뒤 처음 응답했습니다. `first_response_age` 33–35초는 의도한 30초 시작 지연과 요청·관찰 오버헤드가 함께 반영된 값입니다.
>
...
> 👁️ Trial B가 이미 권장 기본값 `Prewarmed=1`로 끝났으므로 정상 완료 경로에서는 별도 PATCH가 필요하지 않습니다. 다음 모듈에 영향을 주지 않도록 인위적인 시작 지연만 삭제합니다.

```bash
# Trial B가 Prewarmed=1 상태이므로 실험 전용 시작 지연만 제거합니다.
...
Prewarmed=0	2026-07-25T07:52:43Z	2026-07-25T07:52:00Z	2026-07-25T07:52:45Z	1
Prewarmed=0	2026-07-25T07:52:43Z	2026-07-25T07:55:00Z	2026-07-25T07:55:23Z	5
Prewarmed=1	2026-07-25T09:13:03Z	2026-07-25T09:13:00Z	2026-07-25T09:13:35Z	1
Prewarmed=1	2026-07-25T09:13:03Z	2026-07-25T09:14:00Z	2026-07-25T09:14:37Z	5
Prewarmed=1	2026-07-25T09:13:03Z	2026-07-25T09:16:00Z	2026-07-25T09:16:40Z	5
```

🟢 **실행 — 결과 표 출력**

> 👁️ 두 JSON 파일을 읽어 Trial A와 B의 부하 시작·최초 응답·프로세스 시작 시각을 하나의 TSV 표로 출력합니다. `load_to_first_response_seconds`가 주 비교 지표이고 `first_response_age`는 30초 시작 지연을 확인하는 보조 지표입니다.
...
jq -r '
  .load_started_at as $load_started_at |
  .observations[] |
  ["Prewarmed=1", .instance, $load_started_at, .first_seen_at,
   (.load_to_first_response_seconds | tostring), .started_at,
   (.first_response_age | tostring)] |
  @tsv
...
Prewarmed=1	d09f4aa4	2026-07-25T09:13:03Z	2026-07-25T09:13:50Z	47	2026-07-25T09:13:15Z	35
Prewarmed=1	c0b2201f	2026-07-25T09:13:03Z	2026-07-25T09:13:52Z	49	2026-07-25T09:13:17Z	35
Prewarmed=1	3122a953	2026-07-25T09:13:03Z	2026-07-25T09:13:57Z	54	2026-07-25T09:13:23Z	34
Prewarmed=1	5bef3ff3	2026-07-25T09:13:03Z	2026-07-25T09:14:05Z	62	2026-07-25T09:13:32Z	33
...
Prewarmed=1	4	47	53	62	15
```

### 관찰된 효과

- **첫 응답:** 최솟값은 51초에서 47초로 4초 줄었습니다.
- **평균·최악 지연:** 평균은 61.8초에서 53초로 8.8초, 최댓값은 68초에서 62초로 6초 줄었습니다.
- **응답 시점의 분산:** 범위는 17초에서 15초로 2초 좁아졌습니다.

이번 실행에서는 Microsoft 권장 기본값 `Prewarmed=1`에서 첫 응답·평균·최악 지연이 모두 낮아졌습니다. 다만 단일 실행 결과이므로 모든 환경에서 같은 개선 폭을 보장하지는 않습니다.
...
따라서 이 실험은 Prewarmed buffer가 없는 상태와 Microsoft가 대부분의 운영 시나리오에 권장하는 기본값 1에서, 부하가 들어온 뒤 신규 capacity가 실제 응답에 얼마나 빠르게 투입됐는지를 비교합니다. 할당된 Prewarmed instance는 초 단위 과금 대상입니다.
...
- **실험 조건 확인:** `first_response_age`가 약 30–40초면 의도한 30초 시작 지연이 적용된 것입니다.
