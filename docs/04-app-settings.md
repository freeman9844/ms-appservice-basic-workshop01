...
> 👁️ **핵심 관찰**: `message`는 기본 문구에서 설정한 문구로 변경되고, `started_at`은 1단계에서 기록한 값과 달라져야 합니다. 이는 앱 설정이 환경변수로 반영되고 그 과정에서 앱 프로세스가 재시작되었음을 의미합니다. 이 `started_at` 관찰 기법은 12 모듈(자동 복구·재시작 관찰)에서 다시 활용됩니다.
