...
> Azure App Service의 핵심 라이프사이클을 **Cloud Shell + az CLI만으로** 체험하는 한국어 핸즈온 워크숍입니다(코어 약 1시간 26분–2시간 5분, 선택 모듈 포함 시 약 1시간 52분–2시간 44분). Python(Flask) 앱을 zip 배포(Oryx 빌드)로 올리고, 앱 설정 → 슬롯 스왑 → 카나리 → 자동 스케일 → 관찰 가능성까지 단계별로 실습합니다. 선택 모듈에서는 **인증(Easy Auth)**, **사이드카 컨테이너**, **Auto-heal & 진단**을 추가로 체험할 수 있습니다.
...
> ✅ **자동 리허설 흐름 제공** — 아래 시간은 각 모듈의 안내·대기 단계를 바탕으로 산정한 예상치입니다. 이전 latency A/B 리허설은 라이브 Azure에서 실행해 보았고, 2026-07-21에 instance-age 흐름도 라이브 리허설을 성공적으로 마쳤습니다. 다만 이번 실행에서는 Prewarmed=0/1 모두에서 비슷하거나 섞인 age가 관찰되어, 단일 실행에서 Prewarmed 승자를 보장하지는 않습니다. 대기 시간이 있는 모듈은 "주 소요 요인" 열을 참고하세요.
...
전체 실습(약 1시간 52분–2시간 44분) 기준 예상 비용은 **USD $1 미만** 수준입니다. 이는 P0v4 1인스턴스 중심 사용(자동 스케일은 최대 5개로 수 분 확장)을 더한 대략치이며, 실습 종료 즉시 정리(12) 수행을 권장합니다. 요금은 리전·통화·시점에 따라 변동될 수 있습니다.
