...
> Azure App Service의 핵심 라이프사이클을 **Cloud Shell + az CLI만으로** 체험하는 한국어 핸즈온 워크숍입니다(코어 약 1시간 30분–2시간 10분, 선택 모듈 포함 시 최대 약 2시간 30분). Python(Flask) 앱을 zip 배포(Oryx 빌드)로 올리고, 앱 설정 → 슬롯 스왑 → 카나리 → 자동 스케일 → 관찰 가능성 → Easy Auth까지 단계별로 실습합니다. 선택 모듈에서는 **사이드카 컨테이너**와 **Auto-heal & 진단**을 추가로 체험할 수 있습니다.
...
두 모듈은 **서로 독립적**이며 골라서 진행할 수 있습니다.

| | 10 사이드카 | 11 Auto-heal & 진단 |
|---|---|---|
| **주제** | Redis 사이드카로 localhost 캐시 | 슬로우 요청 자동 감지·프로세스 재활용 |
| **이런 분께** | 앱 옆에 보조 컨테이너를 붙이는 패턴이 궁금하다 | 운영 중 자가 복구·진단 도구가 궁금하다 |
| **도구** | az CLI (sitecontainers) | az resource update + curl |
| **상태** | GA | GA |
| **소요 시간** | 8–12분 | 8–12분 |
| **유의 사항** | 모듈 09 Easy Auth 활성 시 첫 단계에서 일시 비활성화 필요 | 모듈 09 Easy Auth 활성 시 첫 단계에서 일시 비활성화 필요; 재활용 관찰 대기 약 90초 |

---

## 시간표

> ✅ **리허설 검증 완료** — 아래 시간은 실제 라이브 리허설(자동 실행 기준 전 구간 약 28분)에 학습자 입력·읽기 시간을 더해 산정했습니다. 대기 시간이 있는 모듈은 "주 소요 요인" 열을 참고하세요.
...
| 07 | 자동 스케일 | 12–18분 | 부하 120초 + 확장 ~2분 + 축소 대기 ~5분 |
| 08 | 관찰 가능성 | 10–15분 | 진단 로그 적재 대기 5–10분 |
| 09 | 인증 (Easy Auth) | 10–15분 | 인증 설정 전파 + 브라우저 로그인 |
