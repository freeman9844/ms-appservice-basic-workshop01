...
> ✅ **자동 리허설 흐름 제공** — 아래 시간은 각 모듈의 안내·대기 단계를 바탕으로 산정한 예상치입니다. 이전 latency A/B 리허설은 라이브 Azure에서 실행해 보았지만 결과 변동성이 확인되었고, 현재 instance-age 흐름은 문서·스크립트 기준으로 정리된 상태이며 최종 라이브 리허설이 아직 남아 있습니다. 대기 시간이 있는 모듈은 "주 소요 요인" 열을 참고하세요.
